...
function insertWindow(dirname) {
	...
	let wid = next_wid++; // get the window ID for the new window
	...
	$(":button[id='bar-" + wid + "']").on("click", () => {
		if(win.z_index == 1) { // if win is selected (requires window to be visible)
			win.visible = false
			$(win.jqWin).toggle()
			toBack(win.wid)
		} else if(win.visible){ // if win is visble but not selected
			toFront(win.wid)
...
function redistribute() {
	Object.keys(runningWindows).forEach( key => {
		$(runningWindows[key].jqWin).css("z-index", Object.keys(runningWindows).length-(runningWindows[key].z_index - 1))
		if(runningWindows[key].z_index == 1) {
			$(runningWindows[key].jqWin).children(".title-bar").removeClass("inactive")
		} else {
