...
pub fn getStringPath(self: *FileSystemTree, wd_fd: usize) ![]u8 {
    const serial = self._fd_table.get(wd_fd) orelse return error.BADFD;
    var curr_node: *INode = self.getBySerial(serial);
    var buf: []u8 = &.{};
    if (curr_node.serial_number == self._root) {
        return std.fmt.allocPrint(self._alloc, "/", .{});
    }
    while (curr_node.serial_number != self._root) : (curr_node = self.getBySerial(curr_node.parent)) {
        const tmp_buf: []u8 = try std.fmt.allocPrint(self._alloc, "/{s}{s}", .{ curr_node.name, buf });
        self._alloc.free(buf);
        buf = tmp_buf;
    }
    return buf;
}
...
pub fn stringFDTable(self: *FileSystemTree) ![*:0]usize {
    ...
    buf[99] = 0;
    @panic("stringFDTable() failed, tmp");
    // return buf[0..100 :0];
}
